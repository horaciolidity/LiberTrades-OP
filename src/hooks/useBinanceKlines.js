      ...
      try {
        if (isTiny) {
          ...
          if (Number.isFinite(p)) {
            ...
            // base: barras “constantes” para que no se vea vacío mientras llega WS
            const arr = [];
          ...
          const url = `https://data-api.binance.vision/api/v3/klines?symbol=${symbol}&interval=${interval}&limit=${Math.max(
            limit,
            100
          )}`;
